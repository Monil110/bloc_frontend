...
- 🏗️ `layout.tsx`: The root layout that wraps all pages with the `AppSidebar`.
...
- 🖼️ `layout/AppLayout.tsx`: Orchestrates the main layout structure (Sidebar + Scrollable Content).
...
- 🛠️ `utils.ts`: Helper functions like `cn` for conditional Tailwind class merging.
